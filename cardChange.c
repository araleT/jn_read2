...
void checkCards(int select_cards[8][15], int my_cards[8][15], int select_qty)
{
	...
	{ //各スート毎に走査し
		...
		{ //順番にみて
			...
			if ((my_cards_buf[4][1] == 2 ? count : noJcount) > 2)
			{																	 //3枚以上のとき
	...
	else if (remains > select_qty)
	{ //残りの枚数のほうが交換枚数より多ければ小さいカードからテーブルに追加
		for (j = 0; j < 15; j++)
		{
			if (count == select_qty)
				break;
