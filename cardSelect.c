...
void randomCardDevide(int64 playersCard[5], int64 myCards, int64 oppCards, const changeInfo *cinfo, const fieldInfo *finfo)
{
	...
	if (chg != 0LL)
	{
		...
		for (i = 0; i < 5; i++)
		{
			if (finfo->rank[i] + finfo->rank[finfo->mypos] == 4)
				break;
		}
		playersCard[i] |= chg;
		opp ^= chg;
	...
	if (cinfo->chgCards != 0LL)
	{
		...
		if (((opp >> 52) & 1) == 1LL)
		{
			int cnt = 0;
			for (i = 0; i < 5; i++)
				if (finfo->rank[i] < 3 && i != finfo->mypos)
					cnt += card[i];
			sum = 0;
			pos = genrand_int32() % cnt;
			for (i = 0; i < 5; i++)
			{
				if (finfo->rank[i] > 2 || i == finfo->mypos)
					continue;
				sum += card[i];
				if (pos < sum)
					break;
		...
		if (((opp >> 12) & 1) == 1LL)
		{
			int cnt = 0;
			for (i = 0; i < 5; i++)
				if (finfo->rank[i] < 4 && i != finfo->mypos)
					cnt += card[i];
			sum = 0;
			pos = genrand_int32() % cnt;
			for (i = 0; i < 5; i++)
	...
	if ((opp & (cinfo->notCards)) != 0LL)
	{
		for (i = 0; i < 52; i++)
		{
			if ((((opp & (cinfo->notCards)) >> i) & 1) == 0LL)
				continue;
			int cnt = 0;
			for (j = 0; j < 5; j++)
				if (finfo->rank[j] + finfo->rank[finfo->mypos] != 4 && j != finfo->mypos)
					cnt += card[j];
			sum = 0;
			pos = genrand_int32() % cnt;
			for (j = 0; j < 5; j++)
...
// ランダムにカードを分配する
void randomCardDevide2(int64 playersCard[5], int64 myCards, int64 oppCards, const changeInfo *cinfo, const fieldInfo *finfo)
{
	int i, j;
	int pos, sum, count = 0;
	int card[5];
	...
	// 残りのカードをランダムに分配する
	// opp [100010101..], card[j]:player j's num, count: all opp cards
	// div[j][53]: player j's card dist
...
double calcValueUnrollVH(int64 myCards, int rev, bitValidHandsArray *vhS)
{
	...
	{
		if ((vhS->hands[i].hands & myCards) != vhS->hands[i].hands)
			continue;
		vhSize++;
	...
	// JOKERの検出.
	if (myCards & (1LL << 52))
		res += param[handsSize * rev + jokerIndex];
	return res;
}
		...
		if (simField->lest[*current] == vhb->qty)
		{
			if (targetPlayer == -1)
			{
				...
				if (count == 3)
				{
					for (j = 0; j < 5; j++)
					{
						if (((simField->goal) & (1 << j)) || j == *current)
							continue;
						result[j] = 1;
						return 1;
					}
...
int getActionIndex(const bitValidHandsArray *vhb, const bitValidHandsArray *vhS, int64 myCard, int curRev, int lest)
{
	...
	{
		int rev = curRev;
		if (vhb->hands[j].qty > 3 + vhb->hands[j].seq)
			rev = !rev;
...
int lastSearch(int64 myCards, int64 oppCards, int current, const fieldInfo *finfo, int many, int depth)
{
	...
	for (i = 0; i < vha.size; i++)
	{
		...
		{
			// 子ノードに必勝手が1個でもあれば必勝
			if (lastSearch(playersCard[current], oppCards, curPlayer, &simField, many, depth + 1) == 1)
				return 1;
		}
		else
		{
			// 負けになる手が1個でもあれば負け
			if (lastSearch(myCards, playersCard[current], curPlayer, &simField, many, depth + 1) == 0)
				return 0;
		}
	}
	// 自分：子ノードに勝てる手がない→負け
...
int lastSearchTop(int64 myCards, int64 oppCards, const fieldInfo *finfo)
{
	...
	for (i = 0; i < vha.size; i++)
	{
		...
		if (lastSearch(playersCard[finfo->mypos], oppCards, current, &simField, many, 1) == 1)
			return i;
	}
	return -1;
}
...
double dfunc(double p, double q)
{
	...
	comment_display("dfunc", (p * log(p / q) + (1.0 - p) * log((1.0 - p) / (1.0 - q))));
	...
	if (decFlag == -1)
	{
		for (i = 0; i < SIMULATION_COUNT; i++)
		{
			...
			for (j = 0; j < vha.size; j++)
			{
				...
				{
					muj = (double)simulateScore[j] / (double)simulateCount[j];
				}
			...
			for (j = 0; j < vha.size; j++)
			{
				...
				{
					//IMED
					Nj = (double)simulateCount[j];
					muj = (double)simulateScore[j] / (double)simulateCount[j];

					comment_display("j", (double)j);
					comment_display("Nj", Nj);
					comment_display("muj", muj);
				}

				score[j] = Nj * dfunc(muj, maxj) + log(Nj);
			}
			...
			for (j = 0; j < vha.size; j++)
			{
				...
				comment_display("idx", (double)idx);
				comment_display("j", (double)j);
				comment_display("score[idx]", score[idx]);
				comment_display("score[j]", score[j]);
				if (score[idx] > score[j])
					idx = j; // Imed を「最小」にするハンドを選ぶ
			}

			comment_display("idx is", (double)idx);
			...
			randomCardDevide2(playersCard, myCards, oppCards, cinfo, finfo);
#endif
